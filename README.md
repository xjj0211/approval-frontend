...
* **🎭 动态表单引擎**：摒弃硬编码，支持通过后端 Schema 动态渲染表单组件（Input/Select/Date...），实现低代码能力的初步探索 。
* **🔐 角色权限管理 (RBAC)**：支持“申请人”与“审批员”双角色无缝切换，不同角色拥有严格的操作权限隔离（如仅审批员可见通过/驳回按钮）。
* **📂 附件全流程支持**：实现了图片预览、多文件上传、Excel 模板下载及 Base64 数据流转 。
